...
--- Gets the mode color from style_a.bg_mode without mutating the global config.
--- @param mode Mode The mode of the active tab.
--- @return Color color Color for the given mode.
...
local function set_component_style(component, component_type, mode_color)
	if component_type == ComponentType.A then
		local style = {}
		for k, v in pairs(Yatline.config.style_a) do
			style[k] = v
		end
		if mode_color then
			style.bg = mode_color
		end
		apply_style_table(component, style):bold()
...
local function connect_padding(component, component_type, in_side, component_style)
	local inner = ui.Span(string.rep(" ", Yatline.config.padding.inner))
	local outer = ui.Span(string.rep(" ", Yatline.config.padding.outer))

	local style = component_style
	if not style then
		if component_type == ComponentType.A then
			style = Yatline.config.style_a
		elseif component_type == ComponentType.B then
			style = Yatline.config.style_b
		else
			style = Yatline.config.style_c
		end
	end

	local mode_color = component_type == ComponentType.A and style.bg or nil
	set_component_style(inner, component_type, mode_color)
	set_component_style(outer, component_type, mode_color)
...
function Yatline.string.create(string, component_type, component_name, style)
	local span = ui.Span(string)

	local base_style
	if component_type == ComponentType.A then
		base_style = Yatline.config.style_a
	elseif component_type == ComponentType.B then
		base_style = Yatline.config.style_b
	else
		base_style = Yatline.config.style_c
	end

	local effective_style
	if component_name == "tab_mode" and component_type == ComponentType.A then
		effective_style = { fg = base_style.fg, bg = get_mode_color(cx.active.mode) }
	elseif style then
	...
	apply_style_table(span, effective_style)
	if component_type == ComponentType.A then
		span:bold()
	end

	return ui.Line({ span }), effective_style
end
...
function Yatline.line.create(line, component_type, component_name, style)
	local base_style
	if component_type == ComponentType.A then
		base_style = Yatline.config.style_a
	elseif component_type == ComponentType.B then
	...
	return line, style or base_style
end
...
function Yatline.line.get:tabs(side, style)
	side = side or "left"
	...
	for i = 1, tabs do
		...
		if i == cx.tabs.idx then
			local active_style = style or { fg = Yatline.config.style_a.fg, bg = get_mode_color(cx.tabs[i].mode) }
			local tab = connect_padding(text, ComponentType.A, in_side, active_style)
			set_component_style(tab, ComponentType.A, active_style.bg)

			if active_style.bg ~= "reset" or Yatline.config.show_background then
				separator_style.fg = active_style.bg
				...
				separator_style.fg = active_style.fg
			...
			if i == cx.tabs.idx - 1 then
				local next_mode_color = get_mode_color(cx.tabs[i + 1].mode)

				local open, close
				if
					next_mode_color ~= "reset"
				...
				then
					if
						...
						or (Yatline.config.show_background and Yatline.config.style_c.bg == "reset")
					then
						separator_style.fg = next_mode_color
						if Yatline.config.show_background then
							separator_style.bg = Yatline.config.style_c.bg
						end
						...
						separator_style.bg = next_mode_color
						if Yatline.config.show_background then
...
--- @see set_mode_style To know how mode style selected.
--- @see set_component_style To know how component style applied.
function Yatline.coloreds.create(coloreds, component_type, component_name, style)
	local base_style
	if component_type == ComponentType.A then
		base_style = Yatline.config.style_a
	elseif component_type == ComponentType.B then
	...
	local effective_style = style or base_style
	...
	return ui.Line(spans), effective_style
end
		...
		if component_group then -- Does component group exist?
			if component.custom then -- Does component is custom?
				if component.name ~= nil and component.name ~= "" and #component.name ~= 0 then -- Does component name is valid?
					local line, effective_style = component_group.create(component.name, component_type, component.name, component.style)
					section_components[#section_components + 1] = -- Insert component to the table.
						{ line, component_group.has_separator, effective_style }
				end
			else
				...
				if getter then -- Does function exist?
					...
					if component.params then -- Does component function has parameters?
						if component.type == "line" then
							output = getter(component_group.get, table.unpack(component.params), component.style)
						else
							output = getter(component_group.get, table.unpack(component.params))
						end
					else
						if component.type == "line" then
							output = getter(component_group.get, nil, component.style)
						else
							output = getter()
						end
					end
					...
					if output ~= nil and output ~= "" then -- Does component is not empty?
						local line, effective_style = component_group.create(output, component_type, component.name, component.style)
						section_components[#section_components + 1] = -- Insert component to the table.
							{ line, component_group.has_separator, effective_style }
					end
				end
